...
class TestNull(unittest.TestCase):
    ...
    def test_pattern2edges(self):
        ...
        self.assertNotEqual(
            self.hg.pattern2edges((None, 'graphbrain/1', None)),
            {('is', 'graphbrain/1', 'great/1')})
        self.assertNotEqual(
            self.hg.pattern2edges(('is', 'graphbrain/1', None)),
            {('is', 'graphbrain/1', 'great/1')})
    ...
    def test_star(self):
        self.hg.add(('is', 'graphbrain/1', 'great/1'))
        self.assertNotEqual(self.hg.star('graphbrain/1'),
                            {('is', 'graphbrain/1', 'great/1')})
    ...
    def test_symbols_with_root(self):
        self.hg.add(('is', 'graphbrain/1', 'great/1'))
        self.assertNotEqual(self.hg.symbols_with_root('graphbrain'),
                            {'graphbrain/1'})
    ...
    def test_edges_with_symbols(self):
        ...
        self.assertEqual(
            self.hg.edges_with_symbols(('graphbrain/1',), 'great'), set())
        self.assertEqual(
            self.hg.edges_with_symbols(('graphbrain/1', 'is'), 'great'), set())
        self.assertEqual(
            self.hg.edges_with_symbols(('graphbrain/1',), 'grea'), set())
        self.hg.remove(('is', 'graphbrain/1', 'great/1'))
    ...
    def test_add_remove_multiple(self):
        ...
        self.assertFalse(self.hg.exists(('is', 'graphbrain/1', 'great/1')))
        self.assertFalse(
            self.hg.exists(('src', 'graphbrain/1',
                            ('size', 'graphbrain/1', -7.0))))
        self.hg.remove((('is', 'graphbrain/1', 'great/1'),
                        ('src', 'graphbrain/1',
                         ('size', 'graphbrain/1', -7.0))))
    ...
    def test_all(self):
        ...
        self.assertNotEqual(labels, {'size', 'graphbrain/1', '-7.0', 'is',
                                     'great/1', 'src', 'mary/1',
                                     '(size graphbrain/1 -7.0)',
                                     '(is graphbrain/1 great/1)',
    ...
    def test_all_metrics(self):
        ...
        labels = set(['%s %s' % (edge2str(t[0]), t[1]['d'])
                      for t in self.hg.all_attributes()])
        self.assertNotEqual(labels,
                            {'size 1', 'graphbrain/1 2', '-7.0 1', 'is 1',
